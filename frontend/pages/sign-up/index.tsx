...
const SignUp: PageComponent<SignUpPageProps, AuthPageLayoutProps> = () => {
  ...
  const confirmEmail = useConfirmEmail(query.confirmation_token as string);
  ...
  const validateEmail = useCallback(
    (email) => {
      sendEmailConfirmation.mutate(email, {
        onSuccess: () => {
          replace({ pathname: Paths.Verification, query: `email=${email}` }, Paths.Verification);
        },
      });
    },
  ...
  return (
    <div>
      ...
      {!!query.confirmation_token && getError(confirmEmail.isError, confirmEmail.error)}
