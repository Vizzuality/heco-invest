...
RSpec.describe Impacts::CalculateForProject do
  subject { described_class.new project, demands_source: demands_source }

  describe "#call" do
    context "when demands are loaded from locations" do
      let(:demands_source) { Location }
      let(:project) do
        create :project,
    ...
    context "when demand is taken from project" do
      let(:demands_source) { Project }
      let(:project) do
        create :project, impact_areas: %w[conservation restoration pollutants-reduction water-capacity-or-efficiency gender-equality-jobs]
      end

      before do
        project.update! municipality_biodiversity_demand: 0.7,
          municipality_climate_demand: 0.8,
          municipality_water_demand: 0.7,
          municipality_community_demand: 0.2,
          hydrobasin_biodiversity_demand: 0.1,
          hydrobasin_climate_demand: 0.1,
          hydrobasin_water_demand: 0.1,
          hydrobasin_community_demand: 0.8,
          priority_landscape_biodiversity_demand: 0,
          priority_landscape_climate_demand: 0,
          priority_landscape_water_demand: 0,
          priority_landscape_community_demand: 0
        subject.call
      end
      ...
      it "computes correct hydrobasin impacts" do
        project.reload
        expect(project.hydrobasin_biodiversity_impact).to eq(0)
        expect(project.hydrobasin_climate_impact).to eq(0)
        expect(project.hydrobasin_water_impact).to eq(0)
      ...
      it "computes correct priority landscape impacts" do
        project.reload
        expect(project.priority_landscape_biodiversity_impact).to eq(0)
        expect(project.priority_landscape_climate_impact).to eq(0)
        expect(project.priority_landscape_water_impact.round(10)).to eq(0)
