class Ability
  ...
  def approved_user_rights
    ...
    if user.account.investor_id.present?
      ...
      can %i[funding not_funding], OpenCallApplication, open_call: {investor: {account_id: user.account_id}}
    else
      ...
      can %i[update destroy], OpenCallApplication, project: {project_developer: {account_id: user.account_id}}
    end
  end
end
