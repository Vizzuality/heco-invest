  ...
  module GeoJsons
    class Basins < Base
      ...
      def find_correct_basins_category_for(feature)
        basins_categories.find { |record| record.code == feature.properties["maj_bas"].to_s } || create_basins_category_from(feature)
      end
      ...
      def create_basins_category_from(feature)
        ...
        basins_categories << new_record
        new_record
      end

      def query
        @query ||= Location.where(parent_id: basins_categories.pluck(:id), location_type: :basin).to_a
      end
